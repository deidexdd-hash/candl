...
exports.getMoonPhaseRu = getMoonPhaseRu;
exports.startLunarCron = startLunarCron;
...
function getMoonPhase(date) {
    const { phase } = suncalc_1.default.getMoonIllumination(date);
    if (phase < 0.033 || phase >= 0.967)
        return 'new_moon'; // ~2 дня
    if (phase < 0.22)
        return 'waxing_crescent'; // ~5.5 дней
    if (phase < 0.30)
        return 'first_quarter'; // ~2.5 дня
    if (phase < 0.47)
        return 'waxing_gibbous'; // ~5 дней
    if (phase < 0.53)
        return 'full_moon'; // ~2 дня
    if (phase < 0.72)
        return 'waning_gibbous'; // ~5.5 дней
    if (phase < 0.80)
        return 'last_quarter'; // ~2.5 дня
    return 'waning_crescent'; // ~5 дней
}
function getMoonPhaseRu(phase) {
    const map = {
        new_moon: 'Новолуние',
...
const PHASE_TIPS = {
    new_moon: 'Время посева намерений. Луна невидима — энергия обращена внутрь. ' +
    ...
    waxing_crescent: 'Луна начинает расти — пора делать первые шаги. ' +
        'Начинайте новые дела, делайте первые звонки и встречи. ' +
        'Зелёная или оранжевая свеча поддержат движение и рост.',
    first_quarter: 'Луна на полпути к полнолунию — и путь не всегда прост. ' +
    ...
    waxing_gibbous: 'Лучшее время для практик притяжения — энергия на подъёме перед кульминацией. ' +
        'Зажигайте зелёные и золотые свечи для денег, розовые — для любви. ' +
        'Намерения, поставленные сейчас, достигнут пика на полнолуние.',
    full_moon: 'Кульминация цикла. Не время начинать новое — время завершать и благодарить. ' +
    ...
    waning_gibbous: 'Луна начинает убывать — время отпускать то, что больше не служит. ' +
    ...
    last_quarter: 'Время глубокого прощения и принятия. Что вы готовы отпустить навсегда? ' +
        'Серая или синяя свеча — для мягкого завершения. ' +
        'Чёрная — только если нужна серьёзная работа с тенью или завершение тяжёлого.',
    waning_crescent: 'Луна почти невидима — время тишины и восстановления. ' +
...
function phaseToDbField(phase) {
    if (phase === 'new_moon')
        return 'onNewMoon';
    if (phase === 'full_moon')
        return 'onFullMoon';
    if (['waxing_crescent', 'first_quarter', 'waxing_gibbous'].includes(phase))
        return 'onWaxing';
    if (['waning_gibbous', 'last_quarter', 'waning_crescent'].includes(phase))
        return 'onWaning';
    return null;
}
async function sendLunarNotifications(phase) {
    ...
    const notifications = await index_1.prisma.lunarNotification.findMany({
    ...
    for (let i = 0; i < eligible.length; i += BATCH) {
        await Promise.allSettled(eligible.slice(i, i + BATCH).map((n) => bot.sendMessage(n.user.telegramId.toString(), text, {
            parse_mode: 'Markdown',
...
function startLunarCron() {
    node_cron_1.default.schedule('0 * * * *', async () => {
        try {
            const phase = getMoonPhase(new Date());
            const cronState = await index_1.prisma.cronState.findUnique({ where: { key: 'lunar:last_phase' } });
            const lastPhase = cronState?.value ?? null;
            if (phase === lastPhase)
                return;
            await index_1.prisma.cronState.upsert({
                where: { key: 'lunar:last_phase' },
                create: { key: 'lunar:last_phase', value: phase },
                update: { value: phase },
            });
            const majorPhases = ['new_moon', 'full_moon', 'waxing_gibbous', 'waning_gibbous'];
...
function getLunarToday() {
    ...
    const illumination = suncalc_1.default.getMoonIllumination(now);

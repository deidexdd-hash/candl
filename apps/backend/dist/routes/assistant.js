...
async function assistantRoutes(app) {
    ...
    const geminiClient = GEMINI_KEY ? new generative_ai_1.GoogleGenerativeAI(GEMINI_KEY) : null;
    ...
    app.get('/assistant/usage', async (request) => {
        ...
        const usage = await index_1.prisma.aiUsage.findUnique({
    ...
    app.post('/assistant/ask', async (request, reply) => {
        const { userId } = request.user;
        const user = await index_1.prisma.user.findUnique({ where: { id: userId }, select: { tier: true, firstName: true } });
        const tier = (user?.tier ?? 'free');
        const limit = DAILY_LIMITS[tier];
        if (limit === 0) {
        ...
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const usageRecord = await index_1.prisma.aiUsage.findUnique({
            where: { userId_usageDate: { userId, usageDate: today } },
        });
        ...
        let lunarCtx = '';
        try {
            const l = (0, lunarService_1.getLunarToday)();
        ...
        await index_1.prisma.aiUsage.upsert({
            where: { userId_usageDate: { userId, usageDate: today } },
